...
- **ไม่มี session = กลับไปเข้าระบบที่ SchoolOS** ทั้ง session ที่จบเอง (หมดเวลา /
  ...
  > ⚠️ สามอย่างที่ห้ามถอด ไม่งั้นคนที่ไม่มี session ฝั่ง SchoolOS จะเดินวนระหว่างสองเว็บ
  > โดยไม่มีทางเห็นฟอร์มเลย (บัญชี **admin local ซึ่งเป็นทางเข้าสำรองตอน SchoolOS ล่ม** ·
  > ครู/นักเรียนที่กรอกรหัสเอง · ตอน `npm run dev` ที่ยังไม่ได้ชี้ base มาที่ Users จริง)
  > - **เด้งได้ก็ต่อเมื่อ `GET /api/auth/session` ตอบ `valid:false` เท่านั้น** — "ถามไม่ได้"
  >   (`null`: Users ล่ม / เน็ตกระตุก / CORS / 429 จาก rate limit ของ handoff) ไม่ใช่
  >   "ไม่มีใครล็อกอิน" · กฎเดียวกับที่ `SessionGuard` ใช้ ห้ามเดาจากการที่ขอโค้ดไม่สำเร็จ
  > - `bouncedToSchoolOSRecently()` — เด้งได้รอบเดียวต่อ 10 นาที รอบถัดไปเห็นฟอร์ม
  >   (ธงถูกล้างตอนล็อกอินสำเร็จ จึงนับเฉพาะช่วงที่ยังเข้าไม่ได้จริง ๆ)
...
- **ปุ่ม "ออกจากระบบ" ไปไกลกว่านั้น** — กดออกเอง = ตั้งใจออกจากแพลตฟอร์มด้วย จึง navigate
  ...
  ยิง POST ทิ้งไว้แล้วรีบเปลี่ยนหน้า ซึ่งเบราว์เซอร์ยกเลิกกลางคันได้)
